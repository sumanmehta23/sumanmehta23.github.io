                                                    ...
                                                    {{-- {{ dd($acc) }} --}}
                                                        <div class="col-lg-6 col-xl-4 mb-2">
                                                            <div class="auth-option">
                                                                ...
                                                                <input type="radio" data-group="{{ $acc->ac_name }}" data-inquiry="{{ $acc->inquiry_status }}" class="btn-check acc-types" @if ($i == 0 && !\Carbon\Carbon::parse($acc->competition_start_date, 'UTC')->gte(now('UTC'))) checked @endif name="options" id="option{{ $acc->ac_index }}" value="{{ $acc->id }}" @if ($i == 0 && !\Carbon\Carbon::parse($acc->competition_start_date, 'UTC')->gte(now('UTC'))) disabled @endif> <label class="auth-megaoption @if($i == 0 && !\Carbon\Carbon::parse($acc->competition_start_date, 'UTC')->gte(now('UTC'))) opacity-50 @endif" for="option{{ $acc->ac_index }}" style="height: 230px !important;">
