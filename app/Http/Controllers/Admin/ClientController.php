...
{
    // Retrieve user and related data with eager loading for better performance
    $id = $request->input('userId');
    $user = User::with(['ib', 'ib.category', 'accountTypes.mt5Group'])  // Eager load related models
        ->where('id', $id)
        ->firstOrFail();

    // Retrieve account groups and types in one go
    ...
    // Check if user exists, as we already fetched it
    $eid = $user->id;
    $clients = [];

    // Retrieve IB clients efficiently in a loop
    if ($user->ib) {
        for ($i = 1; $i <= 15; $i++) {
            $clients[$i] = IbClientList::where("ib$i", $user->ib->referral_code)->get();
        }
    }

    // Efficiently fetch total wallet deposit, withdrawal, and pending withdrawal
    ...
    $total_ww = WalletWithdraw::where('user_id', $eid)
        ...
        ->where('status', 1)
        ->sum(DB::raw('withdraw_amount + COALESCE(withdraw_transaction_fee, 0)'));
    ...
    // Retrieve total balance with necessary aggregations
    ...
    // Retrieve related details like bank, KYC, IB, RM in single queries
    ...
    // Fetch ticket status and types in one go
