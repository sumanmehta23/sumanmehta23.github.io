...
class MT5Accounts extends Controller
{
    ...
    public function sendMail($new_user, $type, $platform)
    {
        ...
        $headers .= 'From:' . $settings['admin_title'] . '<' . $from . '>' . "\r\n";
        $content = '
                        <p>Your ' . ucfirst($platform) . ' account is ready! You are all set to dive into the exciting world of trading.</p>
                        <p>Here are your ' . ucfirst($platform) . ' account details</p>
                    ';
        $templateVars = [
            'name' => $new_user->Name,
            'type' => $type,
