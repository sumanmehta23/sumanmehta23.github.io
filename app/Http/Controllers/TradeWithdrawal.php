                ...
                foreach ($promos as $promo) {
                    ...
                    if ($amount_to_deduct > 0) {
                        ...
                        if ($promo->bonus_used > 0) {
                            $depositamountofbonusused = $promo->bonus_used / ($promo_percentage_value / 100);
                        }
                        ...
                        if ($promo_deduction > 0) {
                            ...
                            if($mt5account->Balance > 0){
                                ...
                                Log::alert(" $account->leverage * ((($promo->bonus_amount - $amount) / (($trade_user->Balance + $trade_user->Credit) - $amount))) ");

                                $leverage = round($account->leverage * ((($promo->bonus_amount - $amount) / (($trade_user->Balance + $trade_user->Credit) - $amount))),2);
