...
class MTConnect
{
  ...
  public function __construct($ip_mt5, $port_mt5, $timeout_connection, $is_crypt)
  {
    ...
    $this->crypt_iv         = array(); // Initialize as empty array instead of null
  }
  ...
  private function CryptPacket($packet_body, $len_packet, &$len_crypt_packet)
  {
    ...
    if ($this->m_crypt_out == null) {
      // Check if crypt_iv is initialized and has required elements
      if (!is_array($this->crypt_iv) || !isset($this->crypt_iv[0]) || !isset($this->crypt_iv[1]) || !isset($this->crypt_iv[2])) {
        // Log error and return null to prevent the crash
        if (MTLogger::getIsWriteLog()) MTLogger::write(MTLoggerType::ERROR, "crypt_iv is not initialized properly");
        $len_crypt_packet = 0;
        return null;
      }
  ...
  private function DeCryptPacket($packet_body, $len_packet)
  {
    ...
    if ($this->m_crypt_in == null) {
      // Check if crypt_iv is initialized and has required elements
      if (!is_array($this->crypt_iv) || !isset($this->crypt_iv[0]) || !isset($this->crypt_iv[1]) || !isset($this->crypt_iv[3])) {
        // Log error and return null to prevent the crash
        if (MTLogger::getIsWriteLog()) MTLogger::write(MTLoggerType::ERROR, "crypt_iv is not initialized properly for decryption");
        return null;
      }
